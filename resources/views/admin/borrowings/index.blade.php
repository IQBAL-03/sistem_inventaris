<x-app-layout>
    <x-slot name="header">
        <h2 class="font-black text-2xl text-gray-900 leading-tight uppercase tracking-tighter">
            Lending <span class="text-indigo-600">Operations</span>
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            @if(session('success'))
                <div class="mb-8 glass-card border-l-4 border-green-500 p-4 animate-float">
                    <p class="font-bold text-green-700">Action Successful</p>
                                        ...
                                        <td class="px-6 py-6 whitespace-nowrap text-right space-x-2">
                                            @if($borrowing->status == 'pending')
                                                <form action="{{ route('admin.borrowings.approve', $borrowing) }}" method="POST" class="inline">
                                                    @csrf
                                                    <button type="submit" class="px-4 py-2 bg-green-50 text-green-600 rounded-xl hover:bg-green-600 hover:text-white transition-all font-bold text-[10px] uppercase">Approve</button>
                                                </form>
                                                <form action="{{ route('admin.borrowings.reject', $borrowing) }}" method="POST" class="inline">
                                                    @csrf
                                                    <button type="submit" class="px-4 py-2 bg-red-50 text-red-600 rounded-xl hover:bg-red-600 hover:text-white transition-all font-bold text-[10px] uppercase">Reject</button>
                                                </form>
                                            @elseif($borrowing->status == 'approved')
                                                <form action="{{ route('admin.borrowings.returned', $borrowing) }}" method="POST" class="inline">
                                                    @csrf
                                                    <button type="submit" class="px-4 py-2 bg-indigo-50 text-indigo-600 rounded-xl hover:bg-indigo-600 hover:text-white transition-all font-bold text-[10px] uppercase">Mark Returned</button>
                                                </form>
                                            @endif
                                        </td>
                                    </tr>
